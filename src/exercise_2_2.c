    ...
    double const* const A, double const* const B, double* const C)
{
    ...
    double *A_loc = malloc(rows_per_proc * k * sizeof(double));
    double *C_loc = malloc(rows_per_proc * n * sizeof(double));
    ...
    MPI_Bcast((void*)B, k * n, MPI_DOUBLE, 0, MPI_COMM_WORLD);
    
    for (int i = 0; i < rows_per_proc; ++i) {
        for (int j = 0; j < n; ++j) {
            double sum = 0.0;
            for (int p = 0; p < k; ++p) {
                sum += A_loc[i*k + p] * B[p*n + j];
            }
            C_loc[i*n + j] = sum;
        }
    }
